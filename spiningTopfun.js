...
module.exports.deleteSpining = async (req, res) => {
    ...
    if (!mongoose.isValidObjectId(id))
        return res.status(400).send("чег ма ъчйп");
    let deletedSpining = await SpiningTop.findByIdAndDelete(id)
    if (!deletedSpining)
        return res.status(404).send("ма роца сбйбеп тн лжд чег мозйчд")
...
module.exports.updateSpining = async (req, res) => {
    ...
    try {
        ...
        if (!spiningToUpdate)
            return res.status(404).send("ма роца сбйбеп тн чег лжд")
        ...
        res.status(400).send("аа мтглп" + err)
    }
